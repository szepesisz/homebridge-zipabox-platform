...
const fetchCookie = require('fetch-cookie').default || require('fetch-cookie'); // Handles both cases
...
*/
class ZipaboxApi{
  constructor(debug,url,log,user,password,boxSerial){
    ...
    /* Remote, V3 or local cause API answer is not the same...*/
    this.isRemote = false;
    if(this.baseURL == "https://my.zipato.com:443/zipato-web/v2/" || this.baseURL == "https://my3.zipato.com:443/zipato-web/v2/")
      this.isRemote = true;
    this.isV3 = false;
    if(this.baseURL == "https://my3.zipato.com:443/zipato-web/v2/")
      this.isV3 = true;
  ...
  */
  initUser(){
    return new Promise(function(resolve, reject) {
      ...
      .then(fjson)
      .then(function fgetNonce(jsonResponse){ //FIXME : two next method can be quicker coded
        return new Promise(function(resolve,reject){
  ...
  */
  rebootTheBox(){
    return new Promise(function (resolve, reject){
      ...
      /* Check for V3 */
      if(this.isV3){
        this.log.error("No implementation of reboot method for V3 api.");
        resolve(true);
      } // enf if v3
      /* Logout the user */
  ...
  */
  ...
  /*
  selectBox is use for v3 API. Without it the system stay on hold
  */
  selectBox(){
    /* DEBUG */
    this.debug && this.log("[BOXAPI] > Methode selectBox()");
    // Return the next Promise
    return new Promise(function(resolve, reject){
      /* Check for V3 */
      if(this.isV3){
        fetch(this.baseURL + "box/select?serial=" + this.boxSerial,myInitGet)
        .then(this.fstatus.bind(this))
        .then(fjson)
        .then(function returnOfRequest(jsonResponse){
          this.debugFull && this.log("[BOXAPI] [selectBox] jsonResponse : ",jsonResponse);
          this.debugFull && this.log("[BOXAPI] [selectBox] serial : ",jsonResponse.serial);
          if(jsonResponse.serial == this.boxSerial)
            resolve(true);
          else
            reject(false);
        }.bind(this))
      ...
      }else{ // Always return true for other version of API
        resolve(true);
      }
    }.bind(this)); // end Promise
  } // end selectBox
  ...
  */
  getAllAttributes(){
    ...
    // Fix Adress for API version
    var extraUrl = 'attributes';
    if(this.isV3)
      extraUrl = 'attributes/full';
    // Return the next promise chain
    return new Promise(function(resolve,reject){
      fetch(this.baseURL + extraUrl,myInitGet)
      .then(this.fstatus.bind(this))
      .then(fjson)
      .then(function returnArrayOfUUID(jsonResponse){
        this.debugFull && this.log("[BOXAPI] [getAllAttributes] jsonResponse of getAllAttributes :", jsonResponse);
  ...
  getDeviceStatus(uuidDevice,noStatus){ // Return the device Status
    ...
    if(noStatus){ //config say that no device is available > return true
      ...
      return new Promise(function(resolve, reject) {
        ...
        .catch(function manageError(error) {
          if(error.message == "No JSON" || error.type == 'invalid-json'){
            this.debug && this.log("[BOXAPI] [getDeviceStatus] Invalid JSON manage to force true - Thanks V3. Accessory fix as status ok");
            resolve(true);
          }else{
            this.log.error('[BOXAPI] [getDeviceStatus] Error occurred (message) :', error.message);// TODO ADD gestion Error
            this.log.error('[BOXAPI] [getDeviceStatus] Error occurred (type) :', error.type);// TODO ADD gestion Error
            this.log.error('[BOXAPI] [getDeviceStatus] Error occurred :', error);// TODO ADD gestion Error
            reject(error);
          }
        }.bind(this));// end fetch chaining
      }.bind(this));// End Promise
  ...
  */
  isOnSector(uuidDevice){
    return new Promise(function(resolve,reject){
      ...
      .catch(function manageError(error) {
        if(error.message == "No JSON" || error.type == 'invalid-json'){
          this.debug && this.log("[BOXAPI] [isOnSector] Invalid JSON manage to force true - Thanks V3. Accessory fix as on sector.");
          resolve(true);
        }else{
          this.log.error('[BOXAPI] [isOnSector] Error occurred (message) :', error.message);// TODO ADD gestion Error
          this.log.error('[BOXAPI] [isOnSector] Error occurred (type) :', error.type);// TODO ADD gestion Error
          this.log.error('[BOXAPI] [isOnSector] Error occurred :', error);// TODO ADD gestion Error
          reject(error);
        }
      }.bind(this));// end fetch chaining
    }.bind(this)); // end returned Promise
  ...
  */
  ...
  /*
  putAttributesValue give the specified value for the specified attribute (boolean or value)
  implement the API request /attributes/_uuid_/value (PUT)
  */
  putAttributesValue(uuid,value){
    return new Promise(function(resolve, reject){
      this.debug && this.log("[BOXAPI] > Methode putAttributesValue() uuid :",uuid);
      var myInitPut = {
        method: 'PUT',
        body: value
      };
      this.debugFull && this.log("[BOXAPI] [putAttributesValue] myInitPut:",myInitPut);
      this.debugFull && this.log("[BOXAPI] [putAttributesValue] URL:",this.baseURL + 'attributes/' + uuid + '/value');
      ...
      .catch(function manageError(error) {
        this.log.error('[BOXAPI] [putAttributesValue] Error occurred on putAttributesValue :', error);// TODO ADD gestion Error
        reject(error);
      }.bind(this));// end fetch chaining
    }.bind(this));// End Promise
  }// end putAttributesValue
  ...
  fstatus(response){
    return new Promise(function(resolve,reject){
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        this.log.error("[BOXAPI] [fstatus] Status error on fstatus.", response.statusText);
...
function fjson(response){
  return new Promise(function(resolve,reject){
    var jsonResponseToTest = undefined;
    try{
      jsonResponseToTest = response.json();
    }catch{
      console.error("Error with Json conversion of the response")
      console.log("response :",response);
      console.log("response body :",response.body);
      console.log("response body readable :",response.body.readable);
      reject("No JSON");
    }
    resolve(jsonResponseToTest);
